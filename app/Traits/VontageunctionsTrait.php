...
trait VontageunctionsTrait
{
    ...
    /**
     * Get and process SMS content
     */
    protected function getSmsContent(int $smsProviderId): string
    {
        $precontent = SmsProvider::where('id', $smsProviderId)->value('text');
        $precontent = strip_tags($precontent);
        $precontent = html_entity_decode($precontent);

        return trim($precontent);
    }

    /**
     * Get contact replacements for SMS
     */
    protected function getContactReplacements($contact): array
    {
        return [
    ...
    public function vontagesms_content($sms_provider_id, $contact_id)
    {
        ...
        if ($contact) {
            $replacements = $this->getContactReplacements($contact);

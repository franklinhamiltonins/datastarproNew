...
class ScrapDataLabs extends Model
{
                ...
                foreach ($contacts as $val) {
                    ...
                    if ($call_open_people_search_arr['status']) {
                        ...
                        if (count($contact_scrap_to_be_updated['social_profile_data']) > 0) {
                            $store_social_profile_data[] = array_merge($contact_scrap_to_be_updated['social_profile_data'], ['contact_id' => $val['id']], ['created_at' => date('y-m-d H:i:s')]);
                        }

                        $store_contact_api_platform[] = ['contact_id' => $val['id'], 'status' => $contact_scrap_to_be_updated['status'], 'api_platform_id' => $api_setting_data['id'], 'record_name' => $api_setting_data['platform_name'].'_'.date('y-m-d'), 'merged_status' => $prospect_merged_status, 'created_at' => date('y-m-d H:i:s')];
                        ...
                        $store_contact_api_platform[] = ['contact_id' => $val['id'], 'status' => $status, 'api_platform_id' => $api_setting_data['id'], 'record_name' => $api_setting_data['platform_name'].'_'.date('y-m-d'), 'merged_status' => $prospect_merged_status, 'created_at' => date('y-m-d H:i:s')];
    ...
    public function formatContactScrapUpdate($data, $partial_prospect_status, $contact_data, $api_setting_id)
    {
        ...
        if (count($contact_scrap_to_be_updated) > 0) {
            ...
            $contact_scrap_to_be_updated['is_updated'] = 0;
        }
    ...
    public function formatEmailFields($data)
    {
        ...
        if (count($data['emails']) > 0) {
            foreach ($accepted_mail_providers as $val) {

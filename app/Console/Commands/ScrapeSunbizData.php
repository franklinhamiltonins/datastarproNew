...
class ScrapeSunbizData extends Command
{
	...
	{
		// $pendingBusinesses = Lead::whereIn('sunbiz_status', ['pending', 'failedcrawl'])->where('is_added_by_bot', 1)
		$pendingBusinesses = Lead::
		// whereIn('sunbiz_status', ['checkCrawlData'])->
		where('is_added_by_bot', 1)
		->where("id",">",26206)
			->orderBy('id', 'asc')
			->select('id', 'name')
			->get();
		...
		if ($pendingBusinesses->count() > 0) :
			...
			foreach ($pendingBusinesses as $business) {
				...
				$lead_name = $this->replaceSubstrings(strtoupper($business->name));
			...
			// $bar->finish();
			$this->info($count . ' businesses has been updated successfully.');
		endif;
	}

	function replaceSubstrings($string) {
	...
	public function scrap_sunbiz($lead_name = 'Ocean 14', $lead_id)
	{
		...
		try {
            ...
            Lead::where('id', $lead_id)->update(['sunbiz_status' => 'failedcrawl']);
			return [];
        }
		...
		$crawler->filter('[id^="search-results"]>table>tbody>tr')->each(function ($node) use (&$scrap_response, &$match_found, $lead_name, $entity_name_probability_arr, $lead_id, $list_url) {
			...
			if ($similarity['similarity'] >= 0.5 && !$match_found) {
				...
				$scrap_response['handle_data'] = $this->getcontactDetails($business_name_href, $list_url, $lead_id);

				// echo "<pre>";print_r($scrap_response);exit;

				Lead::where('id', $lead_id)->update(['sunbiz_list_url' => $list_url, 'sunbiz_details_url' => 'https://search.sunbiz.org' . $business_name_href, 'sunbiz_status' => 'crawled']);
				$match_found = true;
			}
		});

		if (empty($scrap_response)) {
			Lead::where('id', $lead_id)->update(['sunbiz_list_url' => $list_url, 'sunbiz_details_url' => '', 'sunbiz_status' => 'failedcrawl']);
		}
		// dd($scrap_response);
		return $scrap_response;
	}

	public function getcontactDetails($url, $list_url, $lead_id)
	{
	    ...
	    foreach ($sections as $section) {
	        ...
	        $sectionCrawler->filterXPath('//text()[normalize-space()]')->each(function ($node) use (&$membersNames) {
	    ...
	    if (count($members) && count($membersNames) && count($members) === count($membersNames)) {
	    	...
	    	for ($i = 0; $i < count($members); $i++) {
				...
				$this->checkContactExistance($lead_id,$first_name,$last_name,$full_name,$members[$i]['member_title']);
	...
	public function getcontactDetails_old($url, $list_url, $lead_id)
	{

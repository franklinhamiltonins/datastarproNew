...
class ImportController extends Controller
{
        ...
        if ($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv') { // if the extension matches, proceed
            ...
            foreach ($fileDataChunks as $fileDataChunk) {
                foreach ($fileDataChunk as $key => $data) {
                    ...
                    } elseif ($contact && $contactSlug) { // if the contact exists store the info message
                    ...
                    } elseif (! $contactFName || ! $contactLName || ! $contactAddress1 || empty($contactSlug)) {
                        $dataErrors->push(
                            [
                                [
                                    'row' => $updated,
            ...
            foreach ($interpolation as $key => $newValue) {
                if ($key == $prop && $newValue) {
                    if (! $contact->$prop) {
                        ...
                        // if($overwrite){
                        if ($overwrite && $newValue != $contact->$prop) {
            ...
            if ($key) { // if the eky is not empty
                switch ($key) {
                    ...
                    case $bussPremium:
                        ...
                        } elseif (! preg_match($twoDecimals, $data[$key], $matches) && ! empty($data[$key])) {
                    ...
                    case $unitCount:
                        ...
                        } elseif (! preg_match($unitCountMax4Dig, $data[$key], $matches) && ! empty($data[$key])) {
                            // validate 9999 digit max 4 numbers
    ...
    public static function format_cell_date($data, $key, $dataErrors, $updated, $niceNames)
    {
        ...
        switch ($data[$key]) {
            case preg_match($dayMonthYear, $data[$key], $matches):
                // convert from d/m/y:
            ...
            case preg_match($MonthDay, $data[$key], $matches):
            ...
            case preg_match($monthDayYear, $data[$key], $matches):
                // convert 7/28/1977 to 1977-28-07  (MM-DD-YYYY)
                $data[$key] = preg_replace('/[-!\\\\$%^&*()_+|~=`{}\[\]:";<>?,.\/ ]/', '-', $data[$key]);
                $newDate = explode('-', $data[$key]);
                $data[$key] = $newDate[2].'-'.$newDate[1].'-'.$newDate[0];
                break;
            case preg_match($YearMonthDay, $data[$key], $matches):
                // convert from :
            ...
            case ! preg_match($dayMonthYear, $data[$key], $matches)
            && ! preg_match($MonthDay, $data[$key], $matches)
            && ! preg_match($YearMonthDay, $data[$key], $matches)
            && ! preg_match($monthDayYear, $data[$key], $matches)
            && ! empty($data[$key]):// if the date doesn't match the regez

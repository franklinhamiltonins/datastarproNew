        ...
        <div class="form-group col mb-0">
            <div id="{{ $type }}_div" class="mt-2 otherInput" {!! $isOther ? '' : 'style="display:none"' !!}>
                <input placeholder="{{ $title }} Carrier" class="form-control"
                       name="{{ $type }}_other" type="text" value="{{ $isOther ? $selected : '' }}">
            </div>
        </div>
    </div>
</div>
...
<div class="mb-2">
    <div class="form-row">
        ...
        <div class="form-group col mb-0">
            ...
            <input placeholder="Other Exclusions" class="form-control" name="{{ $type }}_other_exclusions"
                    type="text" value="{{ $otherExclusions ?? '' }}">
        </div>
        @endif
        @if(isset($pricePerUnit))
        <div class="form-group col mb-0">
            <strong>Price Per Unit:</strong>
            <input placeholder="Price Per Unit" class="form-control" name="{{ $type }}_price_per_unit"
                    type="text" value="{{ $pricePerUnit ?? '' }}">
        </div>
        @endif
    </div>
</div>
...
<div class="mb-2">
    <div class="form-row">
        ...
        <div class="form-group col mb-0">
            ...
            {!! Form::select($type . '_pending_litigation', ['No' => 'No', 'Yes' => 'Yes'],
                $pendingLitigation ?? '', ['class' => 'form-control']) !!}
        </div>
        @endif
    </div>
</div>
...
<div class="mb-2">
    <div class="form-row">
        ...
        <div class="form-group col mb-0">
            ...
            {!! Form::select($type . '_elevation_certificate', ['No' => 'No', 'Yes' => 'Yes'],
                $elevationCertificate ?? '', ['class' => 'form-control']) !!}
        </div>
        @endif
        @if(isset($lomaLetter))
        <div class="form-group col mb-0">
            <strong>Loma Letter:</strong>
            {!! Form::select($type . '_loma_letter', ['No' => 'No', 'Yes' => 'Yes'],
                $lomaLetter ?? '', ['class' => 'form-control']) !!}
        </div>
        @endif
    </div>
</div>

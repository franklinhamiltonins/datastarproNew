...
<div class="form-group col mb-0">
    <strong>{{ $label }}:</strong>
    <select name="{{ $name }}" id="{{ $id }}" class="form-control input selectboxcarrier"
            placeholder="Select Carrier" onchange="toggleOtherInput(this, '{{ $carrierId }}')">
    ...
    <div id="{{ $carrierId }}" class="mt-2 otherInput" {!! $isOther ? '' : 'style="display:none"' !!}>
        <input placeholder="Enter {{ strtolower($label) }}" class="form-control"
               name="{{ $name }}_other" type="text" value="{{ $isOther ? $selectedVal : '' }}">
    </div>
</div>

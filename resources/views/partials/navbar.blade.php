            ...
            <li class="nav-item pr-2">
                <div class="impersonate position-relative">
                    <input
                        ...
                        placeholder="Impersonate an agent"
                        name="impersonate"
                        id="impersonate_user"
                    />
        ...
        @if ($notifications["can_access_notification"])
            <li class="dropdown messages-menu open">
                <a href="#" class="dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="true">
                    ...
                    <span class="label badge label-success" id="inboundlist">
                        {{ $notifications["inbound_count"] }}
                    </span>
                </a>
                                ...
                                @foreach ($notifications["inbound_messages"] as $message)
                                    <li>
                                        <h4 class="d-flex align-items-center justify-content-between inbould_name">
                                            <a href="/leads/edit/{{ base64_encode($message->lead_id) }}" class="pb-0">
                                                <span>{{ $message->full_name }}</span>
                                            </a>
                                            <span class="small text-primary">
                                                {{ date("jS M, y, H:i", strtotime($message->in_time)) }}
                                            </span>
                                        </h4>
                                        <a href="/smsproviderlist/1/{{ $message->contact_id }}">
                                            <p class="inbound_content">{{ $message->content }}</p>
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
            ...
            <li class="dropdown messages-menu open">
                <a href="#" class="dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="true">
                    ...
                    <span class="label badge label-success" id="inboundlistnotification">
                        {{ $notifications["inbound_notification_count"] }}
                    </span>
                </a>
                        ...
                        <li>
                            <ul class="menu">
                                @foreach ($notifications["inbound_notification_messages"] as $message)
                                    <li>
                                        <h4 class="d-flex align-items-center justify-content-between inbould_name">
                                            <a href="/newsletter/{{ $message->newsletter_id }}" class="pb-0">
                                                <span>{{ $message->full_name }}</span>
                                            </a>
                                            <span class="small text-primary">
                                                {{ date("jS M, y, H:i", strtotime($message->in_time)) }}
                                        ...
                                        <a href="/smsproviderlist/2/{{ $message->newsletter_id }}">
                                            <p class="inbound_content">{{ $message->content }}</p>
                                        </a>
                                    </li>
                                @endforeach
        ...
        @if (Auth::check() && Auth::user()->hasRole("Super Admin"))
            <li class="dropdown messages-menu open">
                <a href="#" class="dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="true">
                    ...
                    <span class="label badge label-success" id="navbarNotification">
                        {{ $notifications["call_initiated_count"] }}
                    </span>
                </a>
                                ...
                                @foreach ($notifications["call_initiated_coll"] as $contact)
                                    <li>
                                        <a href="/leads/edit/{{ base64_encode($contact->lead_id) }}">
                                            ...
                                            <p>
                                                Please update the status of contact : {{ $contact->c_full_name }}
                ...
                <li class="user-footer">
                    <div class="pull-left float-left">
                        <a
                            ...
                            class="btn btn-sm btn-default btn-flat px-3 py-2"
                        >
                            Profile
                        </a>
                    </div>
                    <div class="pull-right float-right">
                        <a
                            ...
                            "
                            class="btn btn-sm btn-default btn-flat px-3 py-2 d-flex align-items-center"
                        >
                            Sign out

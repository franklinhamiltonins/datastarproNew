...
@section("content")
                    ...
                    <div class="card p-2 p-lg-3 overflow-auto">
                        <div class="d-flex justify-content-between mb-4">
                            ...
                            <div class="d-flex modulePerPageContainer align-items-center">
                                ...
                                @if ($showPerPage)
                                    <div
                                        class="form-group d-flex mb-0 align-items-center"
                                        id="{{ $moduleName }}_perPage"
                                    >
                                        <label for="perPage" class="mb-0">Items per page:</label>
                                        <select
                                            id="{{ $moduleName }}_selectPerPage"
                                            class="form-control moduleSelectPerPage"
                                        >
                                            <option
                                                class="modulePerPageOption"
                                                value="5"
                                                {{ $perPage == 5 ? "selected" : "" }}
                                            >
                                                5
                                ...
                                <a
                                    ...
                                    class="btn btn-sm btn-info action-btn p-1"
                                    id="moduleRefresh"
                                >
                                    <img
                                        src="{{ asset("/images/module/refresh-icon.png") }}"
                                        alt="refresh"
                                        class="white-img"
                                    />
                                </a>
                        ...
                        <div id="{{ $moduleName }}_container" class="moduleTableContainer">
                            <div id="moduleTableProcessing" class="dataTables_processing" style="display: none">
                                <img src="{{ asset("/images/module/loader.gif") }}" alt="Loader" />
                            </div>
                            <table class="moduleTable rounded" id="{{ $moduleName }}_table">
                                <thead id="{{ $moduleName }}_thead" class="moduleHeadContainer">
                                    <tr id="{{ $moduleName }}_head_tr" class="moduleHeadTrContainer">
                                        @foreach ($tableHeaders as $header)
                                            <th
                                                class="modulethead @if($header['columnName']==$sortColumn) selected @endif"
                                                data-sort-order="{{ $sortDirection }}"
                                                data-sort-column="{{ $header["columnName"] }}"
                                            >
                                                {{ $header["niceName"] }}
                                            </th>
                                        @endforeach
                                ...
                                <tbody id="{{ $moduleName }}_tbody" class="modultTbody"></tbody>
                            </table>
                            <div
                                id="paginationLinksContainer"
                                class="pagination-links justify-content-center d-flex mt-4"
                            ></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var dailing_view_pagination_number = getFromSessionStorage('dailing_view_pagination_number');
            var dailing_view_per_page = getFromSessionStorage('dailing_view_per_page');
            ...
            if (dailing_view_pagination_number || dailing_view_per_page) {
                requestData.currentPage = dailing_view_pagination_number;
                requestData.perPage = dailing_view_per_page;

                if (dailing_view_per_page != 25) {
                    var selectPerPage = document.getElementById('{{ $moduleName }}_selectPerPage');
                    selectPerPage.value = dailing_view_per_page;
                }
            }
            ...
            document.getElementById('paginationLinksContainer').addEventListener('click', function (event) {
                ...
                if (event.target && event.target.matches('.paginationLinks')) {
                    ...
                    // Check if the pageUrl contains 'page=' before using the regular expression
                    if (pageUrl.includes('page=')) {
                        var matchResult = pageUrl.match(/page=(\d+)/);

                        // Check if matchResult is not null and has the expected structure
                        if (matchResult && matchResult[1]) {
                            var pageNumber = matchResult[1];
                            setInSessionStorage('dailing_view_pagination_number', pageNumber);
            ...
            document.getElementById('{{ $moduleName }}_selectPerPage').addEventListener('change', function (event) {
                ...
                setInSessionStorage('dailing_view_per_page', event.target.value);
                setInSessionStorage('dailing_view_pagination_number', 1);
                requestData.currentPage = 1;
                loadModuleData(); // Pass the selected value to the AJAX function
            });
            ...
            function generateBodyData(data) {
                ...
                if (data.length === 0) {
                    ...
                    bodyHtml +=
                        '<tr class="tbodyrow noRecordsFound"><td colspan="' +
                        colspan +
                        '" class="tbodycolumns">No records found</td></tr>';
                } else {
                    const responseDataArray = Object.values(data);
                    responseDataArray.forEach(function (item) {
                        bodyHtml += '<tr class="tbodyrow" id="lead_view_custom_' + item.id + '">';
                        bodyHtml += '<td class="tbodycolumns">' + (item.id ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.name ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.city ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.county ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.unit_count ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.queued_at ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.no_of_times_contacts_called ?? '') + '</td>';
                        bodyHtml += '<td class="tbodycolumns">' + (item.prospect_details ?? '') + '</td>';
                        bodyHtml += '</tr>';
                    });
                }
            ...
            // 	actionsHtml += '<a class="btn btn-sm btn-success action-btn p-1 white-img" title="Edit ' + moduleName + '" href="' + generateActionUrl('edit', item.id) + '"><img src="{{ asset(" / images / module / edit - slick.png ") }}" alt="edit" style="width: 18px;"></a>';

            // 	actionsHtml += '<a href="#" title="Delete Dialing List" data-toggle="modal" data-target="#moduledeleteModal" onclick="setModal(this, \'' + item.id + '\', \'' + moduleName + '\')" class="btn btn-sm btn-danger deletebtn action-btn p-1 white-img">';
            // 	actionsHtml += '<img src="{{ asset(" / images / module / delete - slick.png ") }}" alt="Delete" style="width: 18px;">';
            // 	actionsHtml += '</a>';
            ...
            function updatePaginationLinks(links, currentPage) {
                ...
                if (links.length === 0) {
                    ...
                    links.forEach(function (link) {
                        // Check if link.url is null
                        if (link.url === null) {
                            link.url = '#'; // Set a default value or your desired fallback
                            var linkHtml =
                                '<a class="paginationLinks next_prev_disabled" href="' +
                                link.url +
                                '">' +
                                link.label +
                                '</a>';
                            ...
                            var linkHtml =
                                ...
                                (link.label == currentPage ? ' selected' : '') +
                                '" href="' +
                                link.url +
                                '">' +
                                link.label +

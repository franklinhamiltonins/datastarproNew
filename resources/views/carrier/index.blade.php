@extends('layouts.app')
@section('pagetitle')
    @if($pending == 1) Carrier Management 
    @elseif($pending == 2) Carrier Management (Pending Request)
    @endif
@endsection
@push('breadcrumbs')
...
<section class="content">
            ...
            <div class="card-body p-0 pb-3">
                    ...
                    <div class="row">
                        <div class="col-lg-12 margin-tb d-flex flexwrap-wrap justify-content-between table-top-sec">
                            <div class="d-flex flexwrap-wrap action-dropdown">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu p-0 m-0 text-nowrap" aria-labelledby="actionbtn">
                                        {{-- @can('template-delete') --}}
                                        <a href="javascript:void(0)" id="bulk_carrier_remove"
                                            class="bulk_carrier_remove text-nowrap btn-block rounded-left-0 btn btn-danger btn-sm btn-sm closebtn mr-1">
                                    ...
                                    <div id="custom_length_menu">
                                        <label class="d-flex align-items-center justify-content-between mb-0">Show
                                            <select id="customPageLength"
                                                ...
                                                aria-controls="carrier_status_datatable">
                            ...
                            <div class="d-flex flexwrap-wrap">
                                <div id="carrier_status_datatable_filter" class="dataTables_filter search-sec mb-0">
                                    <label
                                        class="d-flex align-items-center justify-content-end mb-0 position-relative"><input
                                            ...
                                            aria-controls="carrier_status_datatable" class="form-control">
                                        <i class="fas fa-search position-absolute"></i>
                                    </label>
                                </div>
                                <div class="d-flex ml-1">
                                    <a title="Create carrier" class="btn btn-success btn-sm d-flex align-items-center justify-content-center create-btn mr-1" href="{{route('carrier.create')}}" style="width: 42px;height: 42px;">
                                        <i class="fas fa-plus-circle"></i>
                                        <!-- <span class="d-none d-md-inline">Create</span> -->
                                    </a>
                                    @if($pending == 1)
                                        <a title="Carrier List" class="btn btn-success btn-sm d-flex align-items-center justify-content-center create-btn" href="{{route('carrier.index', 2)}}" style="width: 42px;height: 42px;">
                                            <i class="fas fa-list"></i>
                                            <!-- <span class="d-none d-md-inline">Create</span> -->
                                        </a>
                                    @else
                                        <a title="Carrier List (Pending Request)" class="btn btn-success btn-sm d-flex align-items-center justify-content-center create-btn"  href="{{route('carrier.index', 1)}}" style="width: 42px;height: 42px;">
                                            <i class="fas fa-list"></i>
                                            <!-- <span class="d-none d-md-inline">Create</span> -->
                                        </a>
                                    @endif
                ...
                <div class="px-3 pt-2">
                    <div class="table-container pb-2">
                        <table class="order-column compact hover searchHighlight carrier_status_datatable" id="carrier_status_datatable">
    ...
    @include('partials.newdelete-modal')
...
jQuery(document).ready(function() {
    ...
    draw_table();
    $('#carrier_status_datatable tbody').on('click', 'tr', function() {
...
function draw_table() {
    ...
    var table = jQuery('#carrier_status_datatable').DataTable({
        // dom: 'lBfrtip',
        processing: true,
        oLanguage: {
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    if(parseInt('{{$pending}}') == 1){
                        let appendata =  `<div class="d-flex justify-content-center action-btns">
                            <a  title="View carrier" class="btn btn-sm  btn-info action-btn m-0 d-flex justify-content-center align-items-center" href="{{url('carrier/show')}}/`+encryptedId+`" >
                                <i class="fa fa-eye"></i>
                            </a>
                        ...
                        return `<div class="d-flex justify-content-center action-btns">
                            <a class="btn btn-sm  btn-success action-btn m-0 d-flex justify-content-center align-items-center" href="{{url('carrier/edit')}}/`+encryptedId+`/2">
    ...
    $(document).on('click','.deleteCarrierBtn',function () {
        ...
        $("#previous_carrier_id").val(data_id);
        $.ajax({
            url: '/carrier/countLeadAssociation',
            method: "POST",
            data: {
                data_id: data_id,
            ...
            success: function(response) {
                ...
                if(response.status){
                    ...
                    if (response.totalcount > 0) {
                        // When there are associated carriers
                        deleteBodyContent.append(`
                            <div class="alert reassign-alert mt-2 p-3 text-right">
                                <div class="d-flex reassign-alert-cntnt text-left">
                                    ...
                                    <p class="mb-2 small">You have `+response.totalcount+` associations with this Carrier, Please reassign those associations before deleting, or, you can proceed without reassigning:</p>
                                </div>
                        ...
                        $.each(response.list, function (key, value) {
                            if (value.count > 0) {
                                let selectBox = $('<select>')
                                    .attr("name", key)
                                    .addClass("carrier-select form-control mb-2");
                                
                                selectBox.append($('<option>').text("Select " + value.name).attr("value", ""));

                                if (value.carrier.length > 0) {
                                    $.each(value.carrier, function (index, carrier) {
                                        selectBox.append($('<option>').text(carrier.name).attr("value", carrier.id));
                                    });
                                }
                                ...
                                // Show warning message if no alternate carrier is available
                                if (value.carrier.length === 0) {
                                    let warningMessage = $("<p>")
                                        .addClass("text-danger my-1 text-xs py-1 px-2 border border-danger rounded warniing-cntnt")
                                        .text("No alternate carrier available for " + value.name + ". Please create a new carrier before reassigning and deleting.");
                        ...
                        // No associated carriers, allow direct delete
    ...
    var $thead = jQuery('#carrier_status_datatable thead #serial_no');
    $thead.prepend('<input type="checkbox" class="select-all">');

    // Select all checkboxes 
    jQuery('#carrier_status_datatable').on('change', '.select-all', function() {
    ...
    jQuery('#carrier_status_datatable').on('change', '.select-row', function() {
    ...
    $('#bulk_carrier_remove').on('click', function() {
        jQuery('#bulk_carrier_remove').prop('disabled', true);
        jQuery('#carrier_status_datatable_processing').show();
        ...
        if (selectedValues.length > 0) {
            ...
            jQuery('#bulk_carrier_remove').prop('disabled', false);
            jQuery('#carrier_status_datatable_processing').hide();
            return false;
        }
    });
    ...
    function deleteSelectedRecords(selectedValues) {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                jQuery('#carrier_status_datatable').DataTable().draw(true);
                jQuery('.select-all, .select-row').prop('checked', false);
            },
            error: function(xhr, status, error) {
...
$('.form-check-input').change(function() {
    ...
    let columnIndex = jQuery('#carrier_status_datatable').DataTable().column(columnValue + ':name').index();
    if (isChecked) {
        jQuery('#carrier_status_datatable').DataTable().column(columnIndex).visible(true);
    } else {
        jQuery('#carrier_status_datatable').DataTable().column(columnIndex).visible(false);
    }
});
</script>
